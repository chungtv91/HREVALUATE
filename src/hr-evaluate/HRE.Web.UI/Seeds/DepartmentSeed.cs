...
{
    public class DepartmentSeed : BaseSeed, ITransientDependency
    {
        private readonly ILogger _logger;
        private readonly HrDbContext _dbContext;

        public DepartmentSeed(ILogger<DepartmentSeed> logger, HrDbContext dbContext)
        {
            _logger = logger;
        ...
        {
            _logger.LogInformation("DepartmentSeed started.");
            var departments = await _dbContext.Departments.ToListAsync(cancellationToken: cancellationToken);
            if (departments.Count > 0) return;

            var records = await ReadExcelTemplateAs(cancellationToken);
            foreach (var record in records)
            {
                var department = departments.SingleOrDefault(x => x.DepartmentName == record.Name);
                department = new()
                {
            ...
            _logger.LogInformation("DepartmentSeed success.");
        }
